...
def main_eval():
    ...
    trigger_log = [[f'trigger_{i+1}', f'trigger_{i+1}@5'] for i in range(len(triggers))]
    ...
    for i, eval_model in enumerate(tqdm(config.eval_models)):
        print(f'Evaluating {i+1}th / {len(config.eval_models)} model...')
        ...
        for text, ground in tqdm(zip(clean_texts, gt_clean)):
            if config.clean_round is not None:
                print('This time, we dont evaluate other classes.')
                break
        ...
        model_res += [clean_acc.compute_metric(), top5clean.compute_metric()]
            ...
            # clean_acc.update(outputs, ground.repeat(10))
            # top5clean.update(outputs, ground.repeat(10))
            # preds = classifier(inputs)
            # pred_labels = torch.argmax(preds, dim=1).detach().cpu().numpy()
            # clean_acc += np.sum(pred_labels == ground) / len(pred_labels)
            # print(np.sum(pred_labels == ground) / len(pred_labels))
            ...
            for text in tqdm(trigger_txt):
                ...
                for n in range(config.runs):
                    ...
                    for key in out:
                        for img in out[key]:
                            input_images.append(img)
                eval_images(input_images, eval_device, classifier, gt, trigger_acc[i], top_trigger_acc[i])
            
            print('Finish trigger{} evaluate.'.format(i+1)) 
            model_res += [trigger_acc[i].compute_metric(), top_trigger_acc[i].compute_metric()]
            
        log[model_name.split('.')[0]] = model_res
        model.cpu()
    ...
    log.to_excel(os.path.join(config.log_save_dir, 'test_3.xlsx'), index=True)
