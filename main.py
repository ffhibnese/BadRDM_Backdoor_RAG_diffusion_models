...
try:
    ...
    if any(arg in {"-h", "--help"} for arg in sys.argv[1:]):
        print(
            "usage: main.py [-h] -b CONFIG [--train true] [--gpus 0,] [--scale_lr false]\n\n"
            "RDM training entrypoint. Full help requires pytorch_lightning; install requirements.txt first.\n"
        )
        raise SystemExit(0)
    missing = exc.name or str(exc)
    raise SystemExit(f"Missing dependency `{missing}`. Run: pip install -r requirements.txt") from exc
