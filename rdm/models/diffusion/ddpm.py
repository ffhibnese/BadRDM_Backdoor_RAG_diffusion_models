...
from attacks import rag_backdoor
...
class MinimalRETRODiffusion(LatentDiffusion):
    ...
    # Retrieve nearest-neighbor embeddings from query patches.
    @torch.no_grad()
    ...
    # Mix conditional and unconditional signals with a mask.
    ...
    def sample_with_query(self, query, cond=None, bs=None, k_nn=None,
                          ...
                          visualize_nns=True, omit_query=False, normalize=False, chosen_nns=None, printer=True,
                          only_caption=False,
                          **kwargs):
        ...
        if self.retriever is not None and self.retriever.searcher is None and not chosen_nns and not only_caption:
            self.train_searcher()
        ...
        if k_nn is None:
            k_nn = self.k_nn # number of neighbors to retrieve
        ...
        if self.nn_encoder is not None:
            ...
            if n_reps is not None:  # Use only text representations.
                nn_patches = torch.stack([query] * n_reps, dim=1)
            else:  # Use one text representation and k-1 nearest-neighbor embeddings.
            ...
            if chosen_nns:
                nn_batch = rag_backdoor.load_chosen_patches(chosen_nns, size=256, k_nn=k_nn)
                nn_batch = rearrange(nn_batch, 'b k h w c -> (b k) c h w').to(self.device)
                print(f'nn_batch: {nn_batch.shape}')
                # Use ClipImageRetriever.forward.
                r_emb = self.retriever.retriever(nn_batch).to('cpu')
                r_emb = r_emb.reshape(4, k_nn, -1)
            elif only_caption:
                r_emb = torch.from_numpy(nn_dict['q_embeddings']).reshape(4, 1, -1)
                # print(r_emb.shape)
            else:
            ...
            print(f'r_emb: {r_emb.shape}, q_emb: {q_emb.shape}')
        ...
        with self.ema_scope("Plotting"):
            samples, _ = self.sample_log(cond=c, batch_size=bs, unconditional_guidance_scale=unconditional_guidance_scale,
                                                      unconditional_conditioning=c_unconditional_guidance, printer=printer, **kwargs)

        x_samples = self.decode_first_stage(samples)
        out["query_samples"] = x_samples

        sys.stdout = sys.__stdout__
        return out, nn_dict['img_ids']
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps,
                   custom_shape=None, del_sampler=False, printer=True, **kwargs):
        ...
        if ddim:
            ...
            samples, intermediates = ddim_sampler.sample(S=ddim_steps, batch_size=batch_size,
                                                         shape=shape, conditioning=cond, verbose=verbose, printer=printer, **kwargs)
            if del_sampler:
                del ddim_sampler
