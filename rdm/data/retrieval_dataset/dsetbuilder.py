...
class DatasetBuilder(object):
    def __init__(self,
        ...
        self.load_backdoor = load_backdoor
        self.chosen_nns = chosen_nns
        ...
        if self.saved_embeddings and not self.chosen_nns and not self.only_caption:
        ...
        # if self.load_backdoor and os.path.isdir(self.searcher_savedir):
        #     shutil.move(self.searcher_savedir, BACKUP)
        ...
        print("!!!!!!!!!searcher save dir")
        print(self.searcher_savedir)
    ...
    def load_embeddings(self):
        ...
        if os.path.isfile(self.saved_embeddings):
            ...
            if self.load_backdoor:
                bds = np.load(self.load_backdoor)
                self.data_pool = {key: np.concatenate([self.data_pool[key], bds[key]], axis=0) for key in self.data_pool}
                print(f'data_pool: {self.data_pool["embedding"].shape}')
        else:
    ...
    def search_k_nearest(self, queries, k=None, is_caption=False,visualize=None, query_embedded=False):
        ...
        if self.chosen_nns or self.only_caption:        
            return {'img_ids': [], 'q_embeddings': query_embeddings_}
        ...
        print(self.data_pool['embedding'].shape)
        # print(f'nns: {nns}')
        if 'class_id' in self.data_pool.keys():
            print(f'classes: {self.data_pool["class_id"][nns]}')
        if 'img_id' in self.data_pool.keys():
            print(f'img_ids: {self.data_pool["img_id"][nns]}')
